...
## 前言
距离上一次发布文章已经 40 多天了。其实在这中间写了好几篇草稿，但是这些草稿一直没有达到能够发表的质量，所以就理所当然的断更了。有些读者可能会好奇在这么长的间歇里面我究竟做了些什么。简单来讲就是 —— 系统的训练算法题，题目包含各大公司的笔试、NOIP 竞赛题、ACM 题（笔试题为主）。题目来源主要有：
1. LeetCode
2. CodeForces
3. Acwing
4. 牛客
...
在学习过程中，我的策略是：
1. 刷各种算法的模板题，并且达到一定的熟练度（什么是熟练度后文有文字和视频讲解）
2. 刷各种算法的应用题

上述策略可以看出，我的目标是熟练掌握各种算法模板和应用（刷题量从来不应该是主要目标），因此这段时间的题量没有多少，大概只有不到 300 题（不过 LeetCode 题量也不小心过了 500 的大关），顺便倍速观摩了 y 总的基础课和提高课（强推 y 总的课，帮我这个曾经的数竞末流选手理顺了不少思路）。

上面讲了这么多废话，只想表示一点，本系列文章是经过本人长时间学习、思考和总结的（尽管仍然很菜）。
本系列文章全部免费，主要包含两个 level：
- 基础 level
- 提升 level

面向的读者按照面试方向大概分为（难度依次递增，会有部分交叉）：
1. 国内中大厂前端社招的算法题
2. 国内中大厂后端社招和校招的算法题
3. Google kickstart 级别题目

> 注意，本篇为算法系列的开篇文章，包含本系列的大纲。

> 本大纲会不断更新，微信公众号以新篇章的方式发布（大版本添补），其他渠道以原文更新为主。

## 正文
算法题一直在中、大厂属于经常考的一项内容，在校招中几乎是必考项。而对于社招而言，无论是前端还是后端，这几年算法考察比例也在提高。而对于想要进入 Google / 巨硬 / airbnb / hulu 这类外企，算法和系统设计题作为必考项已经很久了。

这里，我不太想讨论考察算法的意义所在（尤其很多前端同学会吐槽面试考察算法），面对大环境，往往我们只有两种选择：
1. 努力适应它
2. 足够牛逼，让别人为你改变规则


**首先，很多同学看到算法题就会很慌，但是这类题目真的很难吗？**

个人的经验来看，80% 的算法面试题难度不是很大（尤其是思维的难度，远远低于数学竞赛的思考难度），考察的知识点也都很集中，这些题目可以在我们规律的训练后达到很好的效果。

对于算法题，主要考察两点：
1. 逻辑思维能力
2. 根据思路快速写出代码的能力

在和很多前、后端圈子的同学交流之后，发现很多同学和我最开始一样学习效率比较低，没有系统的知识学习和训练，学习方式很散，最多是通过 tag 集中刷题，这种方式在没有掌握经典模板题时效率是比较低下的。因此，我期望能够通过一系列算法的文章，能够帮助大家更加高效地搞定这些算法面试题。
