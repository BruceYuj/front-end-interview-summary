...
在有限定数量的六种大理石中选取一些大理石，背包大小为 t， 每块大理石体积为 $[1, ... , 6]$，求是否存在方案能够正好装满背包。

这是经典的 **多重背包** 问题，而该类问题有三种解法.

解法一，转换为 01 背包问题：$O(nt)$

$f(i, j)$ 表示前 i 个物品，背包大小为 j 是否能够恰好装满

状态转移方程：
$f(i, j) = f(i-1, j) | f(i-1, j- a_i)$

初始化：$f(0, 0) = True$, 其余为 False.

答案为 $f(n, t)$

通过滚动数组，可以将空间优化到一维空间。

解法二，通过二进制优化转换成 01 背包问题，时间复杂度为 $t*logn$
