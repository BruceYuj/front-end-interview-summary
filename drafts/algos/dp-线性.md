...
延展与思考：
1. 如果要将一个序列 A 转换成非严格单调递增的序列，最小需要改动几个元素？
> 求出当前序列的最长的非严格单调递增子序列的长度为 $x$，然后答案就是 $n-x$

2. 如果要将一个序列 A 转换成严格单调递增序列，最小需要改动几个元素？
> 新建序列 B， $B_i = A_i - i$, 求出 $B$ 序列的最长非严格单调递增子序列的长度为 $x$，然后答案就是 $n-x$

证明：$a_i < a_{i+1} => a_i - i \leq a_{i+1} - (i+1)$，题目也就转换求 $B_i = A_i - i$，求将 B 转换成**非严格单调递增序列**最小需要改动几个元素。


1. [acwing 274. 移动服务](https://www.acwing.com/problem/content/276/)
...
1. [acwing 275. 传纸条](https://www.acwing.com/problem/content/description/277/)

题目要求：

在矩阵中找出两条路径，两条路径没有交点，并且路径之和最大。

首先，我们要证明，如果两条路径存在交点，那么这两条路径一定不是最优解。通过构造法，我们可以构造一条更优的路径。

在实际的dp过程中，交点的值只会计算一次，因此 $a+0 \leq a+b， b>= 0$,所以绕道的解更优。也就可以通过方格取数来解决。

尝试寻找 “阶段”，两条路径都是从点 $(1, 1)$ 走到 $(n, m)$，我们需要确认中间的每一步如何走。我们可以将 **路径长度** 作为阶段（走过的步数）。

设当前已走了 $i$ 步，且两条路径分别位于 $(x_1, y_1), (x_2, y_2)$ 处。

那么 $x_1 + y_1 = x_2 + y_2 = i + 2$。因此，三个维度即可以描述一个状态。

$f(i, x_1, x_2)$ 表示两条路径长度为 i，且第一条走到第 $x_1$ 行，第二条走到 $x_2$ 行，已经取得的最大值。

每条路径有向右、向下两个方向，共 4 种转移。如果移动到一个各格子，只累计一次。

初始化： $f(0, 1, 1) = A(1, 1)$，求 $f(n+m-2, n, n)$

下面我们给出转移方程：$y_1 = i + 2 - x_1, y_2 = i + 2 - x_2$
1. 向右+向右: f(i, x_1, x_2) = f(i-1, x_1, x_2) + A(x_1, y1) + A(x_2, y_2) （如果$x_1 = x_2$ ，只需要加一个）
2. 向右+向下: 同理
3. 向下+向右：同理
4. 向下+向下：同理
...
if __name__ == "__main__":
    ...
    for i in range(1, M):
        g[i][1:N] = map(int, input().split())
        
    f = [[[0] * M for i in range(M) ] for i in range(N+M)]
    ...
    for k in range(2, n+m+1):
        for i in range(1, M):
            for j in range(1, M):
                i1 = k - i
                j1 = k - j
                if i1 >= 1 and i1 < N and j1 >= 1 and j1 < N:
                    f[k][i][j] = max(f[k-1][i][j], f[k-1][i-1][j-1], f[k-1][i][j-1], f[k-1][i-1][j]) + g[i][k-i] + g[j][k-j]
...
1. [acwing 276. I-区域](https://www.acwing.com/problem/content/278/)

题目分析：
1. 求给定矩阵种所有的凸连通块种权值和最大的方案
2. 凸连通块：左端点列号先减后增，右端点列号先增后减

也就是相当于：我们在矩阵种的每一行考虑选择左端点和右端点来构成一个凸连通块。需要考虑：
- 当前已经处理完的行数
- 当前已经有的格子数
- 当前行已选格子的左端点
- 当前行的右端点
...
行数和格子数可以作为 dp 的阶段，每次转移到下一行，行数和格子数对应的会增加

$f(i, j, l, r, x, y)$ 表示已经处理了前 i 行，选择了 j 个格子，其中第 i 行选择了 $[l, r]$，左边界的单调性类型为 x，右边界的单调性为 y。

转移方程为：
1. 第 i 行左边界列号递减，右边界递增 =》 边界处于扩张
$f(i, j, l, r, 1, 0) = \sum_{p=l}^{r}A[i,p]$ + X$


$$
x = 
\begin{cases}
f(i-1, 0, 0, 0, 1, 0) , if \ j = l+r-1 \\
max_{l\leq p \leq q \leq r}f(i-1, j-(r-l+1), p, q, 1, 0), if \ j > l+r-1
\end{cases}
$$
...
$f(i, j, l, r, 1, 1) = \sum_{p=l}^{r}A[i,p] + max_{l \leq p \leq q \leq r}(max_{0\leq y \leq 1}(f(i-1, j-(r-l+1) p, q, 1, y)))$

3. 左递增，右递减
$f(i, j, l, r, 0, 1) = \sum_{p=l}^{r}A[i,p] + max_{l \leq p \leq q \leq r}(max_{0\leq x \leq 1}(max_{0 \leq y \leq 1}(f(i-1, j-(r-l+1) p, q, x, y)))）$

4. 左、右都递增
$f(i, j, l, r, 0, 0) = \sum_{p=l}^{r}A[i,p] + max_{l \leq p \leq q \leq r}(max_{0\leq x \leq 1}(f(i-1, j-(r-l+1) p, q, x, 0)))$

初始化：
$f(i, 0, 0, 0, 1, 0) = 0$, 其余为负无穷。 求 $max(f(i, K, l, r, x, y))$，时间复杂度为 $O(4*NKM^2) = O(10^7)$


1. [acwing 277. 饼干](https://www.acwing.com/problem/content/279/)

从集合角度来考虑 dp 问题。
- 一般问题是在有限集合内求最值或者求个数
- 个别问题是在无限集合内求最值，此时的做法是通过某种性质将无限集合缩小到有限集合，比如《分级》

本题，期望在所有**饼干分配方案**的集合种，找出最小值。此题的思路和《能量石》类似，可以通过**贪心**将集合进行缩小，再 dp 求出最优解。

根据排序不等式，最有方案一定再此子集中：愤怒值大的小朋友分配更多的饼干。

> 排序不等式：假设 $g_1 < g_2 < g_3 < .... g_n$ $a_1 < a_2 < a_3 < ... < a_n$。
- 两两配对相乘，最终结果的最大值一定是 $\sum{g_i * a_i}$
- 最终结果的最小值一定是 $\sum{g_i * a_{n-i}}$

证明反证法即可。

划分方式类似于 《acwing 900. 整数划分》：
- 解法一： 完全背包问题
- 解法二：$f(i, j)$ 表示总和是 $i$, 恰好分成 $j$ 个数的方案数。
  - 划分方式：最小值是 1 - $f(i-1, j-1)$；最小值大于 1 - $f(i-j, j)$

本题的思考方式和解法二类似。

- 状态表示： $f(i, j)$ 表示前 i 个孩子已经分配了 j 个饼干,且分配饼干数单调下降的最小值
- 状态计算：以前 i 个孩子中共有几个分配了一块饼干进行划分
    - 有 0 个小朋友： $f(i, j-i)$
    - 有 1 个 小朋友: $f(i-1, j-1) + (i-1)* g[i]$
    - 。。。 $f(i-k, j-k) + (i-k) * (\sum_{i-k+1 \leq t \leq i}(g[t])$
    - 有 i 个小朋友 

初始化： $f(0, 0) = 0$ 其余为正无穷。


[3549. 最长非递减子序列](https://www.acwing.com/problem/content/description/3552/)

需要知道的定义：

> a 的 **好子序列** 为 a 中的一段先是 1,再是 2，再是 1，再是 2 的子序列（形如`111222111222`）
- 第一部分：第一段 1 的部分
- 第二部分：第一段 2 的部分
- 第三部分：第二段 1 的部分
- 第四部分：第二段 2 的部分
...
翻转一段之后的最长不下降子序列长度等于不翻转时 a 的最长好子序列长度。

因为上述，2、3部分的翻转正好可以构成 $1112222$ 这种形式。

注意看到，好子序列 1、2 部分构成一个不下降子序列，3、4部分构成一个不下降子序列。

因此，我们可以预处理两个子数组，p1, p2. 其中 p1 表示 $[a_1, a_i]$ 的不下降子序列最长长度； p2 表示 $[a_i, a_n]$ 的不下降子序列长度。

这样 $max(p1[i]+p2[i+1])$ 即为所求。

如何快速求出 p1 和 p2 呢？ 这里以 p1 为例：

普通的LIS 的:
- dp：$O(n^2)$
- 贪心+二分：$O(nlogn)$ - 基本符合要求

但是，本题告诉我们只有两种元素，1 和 2。我们可以将 dp 优化到一层循环，将时间复杂度降低到 $O(n)$
- $f[i][0]$ 保存 $[0,i]$ 范围内以 0 结尾的最长子序列的长度
- $f[i][1]$ 保存 $[0,i]$ 范围内以 1 结尾的最长子序列长度

通过滚动数组空间优化，可以将其优化成两个变量表示：f0, f1。

- $a[i] = 0$, $p1[i] = f0+1$, 且更新 f0
- $a[i] = 1$, $p1[i] = max(f[0], f[1]) + 1$, 且更新 f1
...
解法 2：
$f(i, j)$ 表示前 i 个数，且好子序列的最后一个数属于第 j 部分的子序列集合(注意这里的最后一个数不是指的当前的数 $a[i]$)。属性求子序列长度的最大值。

分情况讨论：
- 情况 1： $a_i = 1$, 所以 $a_i$ 不可能在第 $2, 4$ 部分，只能在 1, 3 部分，所以：
    - $f(i, 2) = f(i-1, 2)$
    - $f(i, 4) = f(i-1, 4)$
    - $f(i, 1) = f(i-1, 1) + 1$
    - $f(i, 3) = max(f[i-1][2], f[i-1][3])+1$
- 情况 2：$a_i = 2$:
...
if __name__ == "__main__":
    ...
    for i in range(1, N):
        if a[i-1] == 1:
            ...
            f[i][2] = max(f[i-1][2], f[i-1][1]) + 1
            f[i][4] = max(f[i-1][3], f[i-1][4]) + 1
            f[i][1] = f[i-1][1]
            f[i][3] = f[i-1][3]
    
    print(max(f[n][1], f[n][2], f[n][3], f[n][4]))
```
```python
...
从题目描述来看，题目和 **邮局问题** 很像，给定一组数 $a[0, .., n]$，寻找一个值 x，使得 $\sum |a_i - x|$ 最小。

答案就是 $a$ 排序后的中位数。证明可以通过反证法来证明。

那么，现在扩展到 k 个数。我们知道，dp 问题在进行状态转移（状态划分）时，常用的技巧是枚举最后一个动作来划分求解，比如本题，动作就是最后一个邮箱可以服务的房子范围。

这样，我们设 $f(i, j)$ 表示前 i 个房子，分配 j 个邮箱的最小总和。答案是 $f(n, k)$

$f(i, j) = f(x, j - 1) + cost(x+1, j)$

初始化： $f(i, 1) = cost(1, i)$, 其余初始化为 INF。

$cost(i, j) = cost(i+1, j-1) + p[j] - p[i]$ 来递推得出。

因为 $k <= n$ 的，因此，我们可以剪枝掉部分求解，也就是 $j > i$ 的情况。
...
预处理出所有 $[i, i+k-1]$ 的和，处理方式为固定长度的滑动窗口解法。时间复杂度为 $O(n)$

$f(i, j)$ 表示 $[i, n)$ 范围内找出 j 个不重叠子数组的最大和。

那么 $f(0, 3)$ 就是最大值.

初始化： $f(i, 0) = 0$, 其他的为 -INF

$f(i, j) = max(f(i-1, j), s[i] + f(i+k, j-1))$ 即可。

那么如何来记录答案的路径呢？

这在 dp 当中也是通用的写法。使用 $path(i, j)$ 记录是由哪个状态转移过来的即可。然后有答案倒退，即可得到具体的答案。

比如 $path(i, j)$ 记录使得 $f(i, j)$ 成立的下标是多少,设为 x，那么对应的转移状态就是 $f(x+k, j-1)$。这样循环处理即可得到答案。

循环什么时候截止？
- 知道答案长度，直接根据答案长度判断即可
- 不知道答案具体长度，循环到具体的判断出口，比如最初始的状态即可。
